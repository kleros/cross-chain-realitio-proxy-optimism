...
describe("Cross-chain arbitration with appeals", () => {
    ...
    it("Should cancel arbitration correctly", async() => {
        await expect(
            homeProxy.handleRejectedRequest(questionID, await requester.getAddress())
        ).to.be.revertedWith("Invalid request status");

        const badMaxPrevious = 11;
        await expect(
                foreignProxy
                .connect(requester)
                .requestArbitration(questionID, badMaxPrevious, { value: totalCost })
            )
        ...
        let request = await homeProxy.requests(
            questionID,
            await requester.getAddress()
        );
        expect(request[0]).to.equal(
            1,
            "Incorrect status of the request in HomeProxy after rejection"
        );
        expect(await homeProxy.questionIDToRequester(questionID)).to.equal(
            ZeroAddress,
        ...
        request = await homeProxy.requests(
            questionID,
            await requester.getAddress()
        );
        ...
        expect(newBalance).to.equal(
            oldBalance + toBigInt(arbitrationCost + surplusAmount - 20000), // 20000 is Redstone temp fee assumed for right now
    ...
    it("Should correctly handle failed dispute creation", async() => {
        ...
        await expect(
            foreignProxy
            .connect(other)
            .handleFailedDisputeCreation(questionID, await requester.getAddress(), {
                value: totalCost,
            })
        ...
        await expect(
                homeProxy.handleNotifiedRequest(questionID, await requester.getAddress())
            )
            .to.emit(foreignProxy, "ArbitrationFailed")
            .withArgs(questionID, await requester.getAddress());

        let arbitration = await foreignProxy.arbitrationRequests(
            arbitrationID,
            await requester.getAddress()
        );
        ...
        const newBalance = await requester.provider.getBalance(
            await requester.getAddress()
        );
        expect(newBalance).to.equal(
        ...
        arbitration = await foreignProxy.arbitrationRequests(
            0,
            await requester.getAddress()
        );
        expect(arbitration[0]).to.equal(0, "Status should be empty");
    ...
    it("Should handle the ruling correctly", async() => {
        ...
        await expect(
            homeProxy.reportArbitrationAnswer(
                questionID,
                ZERO_HASH,
                ZERO_HASH,
                ZeroAddress
            )
        ).to.be.revertedWith("Arbitrator has not ruled yet");
    ...
    it("Should correctly reimburse the overpay when ruling is relayed", async() => {
        ...
        ).to.be.revertedWith("Should cover gas fee");
        ...
        newBalance = await other.provider.getBalance(await other.getAddress());
        expect(newBalance).to.equal(
            oldBalance - toBigInt(l2GasPrice) - toBigInt(txFee), // Take only gas price for L2 and txfee
    ...
    it("Should correctly fund an appeal and fire the events", async() => {
        ...
        await expect(
            ...
            .fundAppeal(arbitrationID, 11, { value: 1000 })
        ...
        expect(newBalance).to.equal(
            oldBalance - toBigInt(5000) - toBigInt(txFee),
        ...
        const contributionInfo =
            await foreignProxy.getContributionsToSuccessfulFundings(
                arbitrationID,
                0,
                await crowdfunder1.getAddress()
            );
        expect(contributionInfo[0][0]).to.equal(
            533,
            "Incorrect fully funded answer returned by contrbution info"
        );
        expect(contributionInfo[1][0]).to.equal(
            8500,
        ...
        await expect(
            foreignProxy
            .connect(crowdfunder1)
            .fundAppeal(arbitrationID, 533, { value: appealCost })
    ...
    it("Should correctly create and fund subsequent appeal rounds", async() => {
        ...
        await foreignProxy
            .connect(crowdfunder1)
            .fundAppeal(arbitrationID, 14, { value: 8500 });
        await foreignProxy
            .connect(crowdfunder2)
            .fundAppeal(arbitrationID, 21, { value: 5000 }); // Winner appeal fee is 6500 full.
        ...
        await foreignProxy
            .connect(crowdfunder2)
            .fundAppeal(arbitrationID, 21, { value: 1500 });
        ...
        await arbitrator.giveAppealableRuling(
            2,
            MAX_ANSWER,
            appealCost,
            appealTimeOut
    ...
    it("Should correctly reimburse the overpay when handling failed dispute", async() => {
        ...
        ).to.be.revertedWith("Should cover gasFee");
    ...
    it("Should have correct balance after paying arbitration cost and arbitrum fee", async() => {
        ...
        const tx = await foreignProxy
            .connect(requester)
            .requestArbitration(questionID, maxPrevious, {
                gasPrice: gasPrice,
                value: totalCost,
            });
        txFee = (await tx.wait()).gasUsed * gasPrice;
    ...
    it("Should reimburse after dispute creation in case of overpay", async() => {
        await foreignProxy
            .connect(requester)
            .requestArbitration(questionID, maxPrevious, { value: oneETH }); // Deliberately overpay
        ...
        expect(requesterDeposit).to.equal(
            oneETH - toBigInt(20000),
    ...
    it("Should not fund the appeal after the timeout", async() => {
        ...
        await expect(
            foreignProxy
            .connect(crowdfunder1)
            .fundAppeal(arbitrationID, 21, { value: appealCost })
        ).to.be.revertedWith("Appeal period is over.");
    });
    ...
    it("Should correctly withdraw appeal fees if a dispute had winner/loser", async() => {
        ...
        await foreignProxy
            .connect(requester)
            .fundAppeal(arbitrationID, 50, { value: 4000 });
        await foreignProxy
            .connect(crowdfunder1)
            .fundAppeal(arbitrationID, 50, { value: oneETH });

        await foreignProxy
            .connect(crowdfunder2)
            .fundAppeal(arbitrationID, 5, { value: 6000 });
        await foreignProxy
            .connect(crowdfunder1)
            .fundAppeal(arbitrationID, 5, { value: 6000 });
        ...
        await foreignProxy
            .connect(requester)
            .fundAppeal(arbitrationID, 44, { value: 500 });
        await foreignProxy
            .connect(crowdfunder1)
            .fundAppeal(arbitrationID, 44, { value: 8000 });

        await foreignProxy
            .connect(crowdfunder2)
            .fundAppeal(arbitrationID, 5, { value: 20000 });
        ...
        // Withdraw 0 round.
        await foreignProxy.withdrawFeesAndRewards(
            arbitrationID,
            requesterAddress,
            0,
            50
        ...
        expect(contributionInfo[1][1]).to.equal(
            0,
            "Contribution of crowdfunder1 should be 0"
        );
        contributionInfo = await foreignProxy.getContributionsToSuccessfulFundings(
            arbitrationID,
            0,
            crowdfunder2Address
        );
    ...
    it("Should correctly withdraw appeal fees if the winner did not pay the fees in the round", async() => {
        ...
        await foreignProxy
            .connect(requester)
            .fundAppeal(arbitrationID, 1, { value: 5000 });
        await foreignProxy
            .connect(crowdfunder1)
            .fundAppeal(arbitrationID, 1, { value: 3500 });

        await foreignProxy
            .connect(crowdfunder2)
            .fundAppeal(arbitrationID, 4, { value: 1000 });
        await foreignProxy
            .connect(crowdfunder1)
            .fundAppeal(arbitrationID, 4, { value: 10000 });
